...
export default function ReconcilePage({ budget, transactions: txHook, periods }) {
  ...
  const [importResult, setImportResult] = useState(null) // { matched, unmatched, unmatchedTotal }
  const [applyResult,  setApplyResult]  = useState(null) // result from the RPC function
  ...
  function handleBuildPreview() {
    if (!colMap.dateCol || !colMap.descCol || !colMap.amountCol) { setError('Please select Date, Description, and Amount columns.'); return }
  ...
  async function handleAssignMatch(index, expenseItemId) {
    ...
    setPreview(prev => prev.map((t, i) => i === index
      ? { ...t, matched_expense_id: expenseItemId, matched_score: 1, matched_source: 'manual',
          suggested_category_name: undefined, suggested_pattern: undefined, _showAssignFor: undefined }
      : t))
    if (expenseItem) {
  ...
  function handleAcceptSuggestion(index) {
    ...
    setPreview(prev => prev.map((t, i) => i === index ? { ...t, _showAssignFor: suggestedCat?.id ?? true } : t))
  }
  ...
  async function handleSave() {
    ...
    const unmatchedTotal = toInsert
      .filter(t => !t.matched_expense_id && !t.ignored && t.amount < 0)
  ...
  // ── Apply matched transactions to budget actuals ──────────────────────────
  async function handleApplyToBudget() {
    ...
    if (error) { setError(error.message); return }
    setApplyResult(data)
    // Reload both budget actuals and transaction applied flags
    reloadBudget()
    reloadTx()
  ...
  return (
    <div className="fadein">
      ...
      {/* Unapplied matches banner — shown when there are matched but unapplied transactions */}
      {unappliedCount > 0 && stage === 'bank' && !applyResult && (
        <div className="alert alert-info" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '.5rem' }}>
          <span>
            <strong>{unappliedCount}</strong> matched transaction{unappliedCount === 1 ? '' : 's'} ready to apply to your budget actuals.
          </span>
          <button className="btn btn-p" style={{ fontSize: '.82rem' }} onClick={handleApplyToBudget} disabled={applying}>
            {applying ? <><span className="spinner" style={{ width: 13, height: 13 }} /> Applying…</> : '✓ Apply to budget'}
          </button>
        </div>
      )}
      ...
      <div className="card rec-wizard">
        ...
        {stage === 'bank' && (
          <div className="rec-body fadein">
            <p className="rec-map-hint">Every import needs to be tied to a bank account so we can keep your statements organized and remember each bank's column layout.</p>
            ...
            {csvRows.length > 0 && (
              <div className="rec-sample">
                ...
                <div className="rec-sample-scroll">
                  <table><thead><tr>{csvHeaders.map(h => <th key={h}>{h}</th>)}</tr></thead>
              ...
              {preview.map((tx, i) => {
                ...
                return (
                  <div key={i} className={`rec-tx${tx._skip ? ' rec-tx-skip' : ''}`}>
                    ...
                    {matched && tx.matched_source === 'rule'   && <div className="rec-tx-match rec-tx-match-rule">🎯 Auto-matched from your rules to <strong>{matched.label}</strong></div>}
                    {matched && tx.matched_source === 'fuzzy'  && <div className="rec-tx-match">✓ Matched to <strong>{matched.label}</strong></div>}
                    {matched && tx.matched_source === 'manual' && <div className="rec-tx-match rec-tx-match-rule">✓ Assigned to <strong>{matched.label}</strong> — rule saved</div>}
                    ...
                    {!matched && tx._showAssignFor && (
                      <div className="rec-tx-assign">
                        ...
                        <select className="cell-select" defaultValue="" onChange={e => e.target.value && handleAssignMatch(i, e.target.value)}>
                          <option value="" disabled>Select…</option>
                          {candidates.map(e => <option key={e.id} value={e.id}>{e.label}</option>)}
                        </select>
                      </div>
                    )}
                    {!matched && !tx.suggested_category_name && (
                      <div className="rec-tx-assign">
                        ...
                        <select className="cell-select" defaultValue="" onChange={e => e.target.value && handleAssignMatch(i, e.target.value)}>
                          <option value="" disabled>Select…</option>
                          {allExpenses.map(e => <option key={e.id} value={e.id}>{e.label}</option>)}
                        </select>
                      </div>
                    )}
                  </div>
          ...
          <div className="rec-body fadein">
              ...
              <div className="rec-apply-summary">
                <div className="wiz-summary-grid" style={{ marginBottom: '1rem' }}>
                  <div className="wiz-scard">
                    <div className="wiz-scard-val">{importResult.total}</div>
                    <div className="wiz-scard-lbl">Imported</div>
                  </div>
                  <div className="wiz-scard">
                    <div className="wiz-scard-val" style={{ color: 'var(--green)' }}>{importResult.matched}</div>
                    <div className="wiz-scard-lbl">Matched</div>
                  </div>
                  <div className="wiz-scard">
                    <div className="wiz-scard-val" style={{ color: importResult.unmatched > 0 ? 'var(--gold)' : 'var(--ink3)' }}>
                ...
                {importResult.unmatched > 0 && (
                  <div className="alert alert-info" style={{ fontSize: '.83rem', marginBottom: '1rem' }}>
                    💡 <strong>{fmt(importResult.unmatchedTotal)}</strong> across {importResult.unmatched} transaction{importResult.unmatched === 1 ? '' : 's'} couldn't be matched to a budget item.
                    You can assign them manually in the Recent Transactions list below, then apply again.
                  </div>
                )}
              </div>
            )}
            ...
            {!applyResult ? (
              <div className="rec-apply-box">
                <div className="rec-apply-title">Apply matched transactions to budget</div>
                <p className="rec-apply-desc">
                  This sums all matched transactions and writes their totals to the <strong>Actual</strong> column
                  on your Income, Monthly Expenses, and Yearly Subscriptions tabs — so your Dashboard shows
                  real numbers instead of zeros. Each transaction is marked applied so it won't be counted twice.
                </p>
              ...
              <div className="rec-apply-result fadein">
                <div className="alert alert-success" style={{ marginBottom: '1rem' }}>
                  ✅ Budget actuals updated — {applyResult.transactions_applied} transaction{applyResult.transactions_applied === 1 ? '' : 's'} applied
                  across {applyResult.expense_items_updated + applyResult.income_items_updated} budget line items.
                </div>
                {applyResult.unmatched_count > 0 && (
                  <div className="alert alert-info" style={{ fontSize: '.83rem', marginBottom: '1rem' }}>
                    💡 <strong>{applyResult.unmatched_count}</strong> unmatched transaction{applyResult.unmatched_count === 1 ? '' : 's'} ({fmt(applyResult.unmatched_total)}) still need assignment.
                    Find them in Recent Transactions below and assign them manually.
                  </div>
                )}
      ...
      {recentTx.length > 0 && (
        <div style={{ marginTop: '2rem' }}>
          <div className="sec-hdr">
            ...
            <span className="sec-hint">{transactions.length} total · {transactions.filter(t => t.applied).length} applied</span>
          </div>
                ...
                {recentTx.map(tx => {
                  ...
                  return (
                    <tr key={tx.id} style={{ opacity: tx.applied ? .6 : 1 }}>
                      ...
                      <td>
                        {tx.applied
                          ? <span style={{ fontSize: '.75rem', color: 'var(--ink3)' }}>applied</span>
